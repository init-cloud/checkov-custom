...
class IC_AWS_15(BaseResourceCheck):
    aws_rule11 = list()
    ...
    def scan_resource_conf(self, conf):
        if ('snapshot_id' and 'account_id') in conf.keys():
            ...
            if IC_AWS_15.aws_rule11 is not None:
                for i in IC_AWS_15.aws_rule11:
                    if snapshot_id in i['snapshot_id'] and account_id in i['account_id']:
                        return CheckResult.FAILED
                    else:
                        IC_AWS_15.aws_rule11.append({'snapshot_id': snapshot_id, 'account_id': account_id})
