...
class IC_AWS_5(BaseResourceCheck):
    role_list = list()
    roles_list = list()
    ...
    def scan_resource_conf(self, conf):
        if 'roles' in conf.keys():
            roles = conf['roles'][0]
            IC_AWS_5.roles_list.append(roles)
            for role in roles:
                if role in IC_AWS_5.role_list:
                    return CheckResult.FAILED
            return CheckResult.PASSED
        if 'role' in conf.keys():
            role = conf['role'][0]
            IC_AWS_5.role_list.append(role)
            for roles in IC_AWS_5.roles_list:
                if role in roles:
                    return CheckResult.FAILED
            return CheckResult.PASSED
